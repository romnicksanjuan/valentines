...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        setInterval(function () {
            ...
            const minusDays = 1000 * 60 * 60 * 24 * 91
            const adjusteddays = difference - minusDays
            const days = Math.floor(adjusteddays / (1000 * 60 * 60 * 24));
            const hours = Math.floor((difference % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
            const minutes = Math.floor((difference % (1000 * 60 * 60)) / (1000 * 60));
            const seconds = Math.floor((difference % (1000 * 60)) / 1000);

            const threeMonthsInMs = 1000 * 60 * 60 * 24 * 91.32;
            const adjustedDifference = difference - threeMonthsInMs;
            const years = Math.floor(adjustedDifference / (1000 * 60 * 60 * 24 * 365.25))
            // const months = Math.floor(difference / (1000 * 60 * 60 * 24 * 30))


            const getMonthsDifference = (startDate: string) => {
                ...
                // Convert years to months and calculate the total difference
                let totalMonths = yearsDiff * 12 + monthsDiff;

                // Subtract 3 months first
                totalMonths -= 3;

                // Now divide the remaining months into blocks of 12
                const blocksOf12 = Math.floor(totalMonths / 12);
                const remainingMonths = totalMonths % 12;

                return { blocksOf12, remainingMonths };
            };

            const { remainingMonths } = getMonthsDifference("2021-12-14");
